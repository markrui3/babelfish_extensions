...
public:
	...
	void exitDml_statement(TSqlParser::Dml_statementContext *ctx) override
	{
		...
		/* common routine for select and non-select, must be the first rewrite. */
		...
		/* Add query hints, must be after rewrite */
...
void replaceTokenStringFromQuery(PLtsql_expr* expr, Token* startToken, Token* endToken, const char *repl, ParserRuleContext *baseCtx)
{
	...
	rewritten_query_fragment.emplace(std::make_pair(startIdx, std::make_pair(startToken->getInputStream()->getText(misc::Interval(startIdx, endIdx)), repl ? std::string(repl) : std::string())));
}
